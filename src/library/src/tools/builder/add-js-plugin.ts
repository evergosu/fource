  ...
  return {
    setup(build) {
      build.onResolve({ filter: /^[^.].*/ }, arguments_ => {
        ...
        if (arguments_.kind === 'import-statement' && !arguments_.path.startsWith('.')) {
          return;
        }
      ...
      build.onLoad({ namespace: 'add-js-extension', filter: /.*/ }, async arguments_ => {
        const tsPath = `${arguments_.path}.ts`;

        const contents = await fs.readFile(tsPath, 'utf8');

        // Rewrite imports to add .js extension.
        const rewritten = contents.replaceAll(/from\s+["'](.+?)["']/g, (match, p1: string) => {
          if (p1.startsWith('.') && !p1.endsWith('.js') && !p1.endsWith('.ts')) {
            return `from "${p1}.js"`;
          }
