...
export class BanVoteQueryRepository implements DomainCountById<BanVoteFailureMap> {
   ...
   */
  public countById(storyId: BanVote<'persisted'>['storyId']): Task<number, AggregatePersistenceFailure> {
    return this.persistence.countById(storyId.toString()).mapError(this.errorPolicy.translate('countById'));
  }
}
